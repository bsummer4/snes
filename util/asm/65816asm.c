...
Dll_n **Global_Dlls[7] = \
	{ 
	&Global_File, &Global_Code, &Global_Data \
...
void Do_Directive(File_s *In)
{
	...
	// Comment
	/* */if(!strcmp(ib,"{" ) //++	||!strcmp(ib,"}" )) Dir_Block(In); //++
...
// Assembler Flow
else if(!strcmp(ib,"M" ) //++||!strcmp(ib,"X" ) //++||!strcmp(ib,"Y" ) //++||!strcmp(ib,"m" ) //++||!strcmp(ib,"x" ) //++||!strcmp(ib,"y" )) Dir_Cpu(In); //++
else if(!strcmp(ib,"PC" )) Dir_PC(In); //++
else if(!strcmp(ib,"File" )) Dir_File(In); //++
else if(!strcmp(ib,"LoROM") //++||!strcmp(ib,"HiROM")) Dir_Rom(In); //++
...
void Error(int program, int code, File_s *In, void *data)
{
	...
	switch(program){
	default: 
		Error(0x01,0x03,In,(void*)&program);
	case 0x00: 
		printf("-DEBUG- %s\n",(char*)data); 
		break;
	case 0x01: 
	...
	case 0x02: 
		...
		break;
	case 0x03: 
		printf("Bad Program ID : %i\n",*(int*)data); 
		break;
	case 0x04: 
		printf("Conflicting declarations\n"); 
		Dll_Print((Dll_n*)data); 
		break;
	case 0x05: 
	...
	case 0x07: 
		...
		break;
	case 0x08: 
		printf("Headerless list\n"); 
		break;
	case 0x09: 
		printf("Immediate size mismatch\n"); 
		break;
	case 0x0A: 
		printf("Invalid #Lo/#HiROM"); 
		break;
	case 0x0B: 
		printf("Invalid addressing mode\n"); 
		break;
	case 0x0C: 
		printf("Invalid bin number : %s\n",(char*)data); 
		break;
	case 0x0D: 
		printf("Invalid data : %s\n",(char*)data); 
		break;
	case 0x0E: 
		printf("Invalid dec number : %s\n",(char*)data); 
		break;
	case 0x0F: 
		printf("Invalid directive : %s\n",(char*)data); 
		break;
	case 0x10: 
		printf("Invalid dll data type\n"); 
		break;
	case 0x11: 
		printf("Invalid escape sequence : \\%c",*((char*)data)); 
		break;
	case 0x12: 
		printf("Invalid file name : %s\n",(char*)data); 
		break;
	case 0x13: 
		printf("Invalid file pointer : %lx\n",*((long*)data)); 
		break;
	case 0x14: 
		printf("Invalid find type\n"); 
		break;
	case 0x15: 
		printf("Invalid hex number : %s\n",(char*)data); 
		break;
	case 0x16: 
		printf("Invalid in file : %s\n",(char*)data); 
		break;
	case 0x17: 
	...
	case 0x1C: 
		...
		break;
	case 0x1D: 
		printf("Invalid out file : %s\n",(char*)data); 
		break;
	case 0x1E: 
		printf("Invalid program counter : %lx\n",*((long*)data)); 
		break;
	case 0x1F: 
	...
	case 0x23: 
		...
		break;
	case 0x24: 
		printf("Null file mode\n"); 
		break;
	case 0x25: 
		printf("Null list\n"); 
		break;
	case 0x26: 
		printf("Operand width mismatch : %s\n",(char*)data); 
		break;
	case 0x27: 
		printf("PC decremented\n"); 
		break;
	case 0x28: 
		printf("Program crossed bank boundry : %lx\n",PC); 
		break;
	case 0x29: 
		printf("ROM overflow\n"); 
		break;
	case 0x2A: 
		printf("Undeclared link reference\n"); 
		Dll_Print((Dll_n*)data); 
		break;
	case 0x2B: 
		printf("Undeclared name reference\n"); 
		Dll_Print((Dll_n*)data); 
		break;
	case 0x2C: 
		printf("Undeclared reference\n"); 
		Dll_Print((Dll_n*)data); 
		break;
	case 0x2D: 
		printf("Unresolved name prototype\n"); 
		Dll_Print((Dll_n*)data); 
		break;
	case 0x2E: 
		printf("Usage [#Code][addr][{][name][}][,] ...\n"); 
		break;
	case 0x2F: 
		printf("Usage [#Data][addr][{][name][}][,] ...\n"); 
		break;
	case 0x30: 
		printf("Usage [#Name][addr][name][,] ...\n"); 
		break;
	case 0x31: 
		printf("Usage [#PC][+][addr][,][pad] ...\n"); 
		break;
	case 0x32: 
		printf("Usage [{][+/-][link][}]\n"); 
		break;
	case 0x33: 
		printf("Usage 65816asm input_file [switches]\n"); 
		break;
	case 0x34: 
		printf("Usage [-i][in_file]\n"); 
		break;
	case 0x35: 
		printf("Usage [-o][out_file]\n"); 
		break;
	}
...
/*
	...
	of the number/name operand 'In->line'. If the operand is a name,
	a name reference will be recorded on 'Global_Name_R'.
*/
	...
	case Token_number_t_:
		...
		{	
			...
			{	
				...
				{
				case 2: 
					if(d>1) Error(0x1B,0x0C,In,(void*)ib); 
					break;
				case 10: 
					if(d>9) Error(0x1B,0x0E,In,(void*)ib); 
					break;
				case 16: 
					if(d>15) Error(0x1B,0x15,In,(void*)ib); 
					break; 
				}
			}
...
break;
case 1: 
if(!(0<=In->data&&In->data<=0xFF)) Error(0x1B,0x26,In,(void*)ib); 
break; // Operand width mismatch
case 2: 
if(!(0<=In->data&&In->data<=0xFFFF)) Error(0x1B,0x26,In,(void*)ib); 
break; // Operand width mismatch
case 3: 
if(!(0<=In->data&&In->data<=0xFFFFFF)) Error(0x1B,0x26,In,(void*)ib); 
break; // Operand width mismatch
			}
			break;
		case 16:
		...
		for(n=l->next;n;n=n->next){
			...
			case Dll_link_t_:
				if((n->type==Dll_link_t_)
				    &&(!strcmp(k->name,((Dll_link_s*)n->data)->name)) // Same name				&&(!strcmp(k->file,((Dll_link_s*)n->data)->file)) // Same file				&&((0xFF0000&k->addr)==(0xFF0000&((Dll_link_s*)n->data)->addr)) ){ // Same bank
					o=(0xFFFF&(((Dll_link_s*)n->data)->addr))-(0xFFFF&k->addr);
					if(k->name[0]=='+'&&o<0) // Correct direction					break;
					if(k->name[0]=='-'&&o>0)
						break;
					...
					||k->mode==0xD0 // BNE
					||k->mode==0xF0) // BEQ					&&((o>=0&&o>128)||(o<=0&&o<=-128)))
						continue;
					if(!m)
						m=n;
					else
						if(abs(o)<abs((0xFFFF&(((Dll_link_s*)m->data)->addr))-((0xFFFF&k->addr)))) // Closests						m=n;
				}
				break;
				// Name
			case Dll_name_t_:
