...
int main (int argc, char *argv[]) {
	...
	if (argc<2) Error(0x02,0x33,NULL,NULL);
	if (!Is_Name(argv[1])) Error(0x02,0x16,NULL,(void*)argv[1]);

	for (i=2;i<argc;i++) {

		if (!strcmp(argv[i],"-o")){
			if (s) Error(0x02,0x23,NULL,NULL);
			if (++i>=argc) Error(0x02,0x35,NULL,NULL);
			if (!Is_Name(argv[i])) Error(0x02,0x1D,NULL,(void*)argv[i]);
			if (!(s=(char*)malloc(strlen(argv[i])+1))) Error(0x02,0x07,NULL,NULL);
			s[0]='\0';
			strcat(s,argv[i]);
			continue; }

		if (!strcmp(argv[i],"-i")){
			if (++i>=argc) Error(0x02,0x34,NULL,NULL);
			if (!Is_Name(argv[i])) Error(0x02,0x16,NULL,(void*)argv[i]);
			n=Dll_New(Dll_file_t_);
			strcat(((Dll_file_s*)n->data)->name,argv[i]);
			Dll_Insert(Global_File,n);
			continue; }

		if (!strcmp(argv[i],"-r")) { Report_f=1; continue; }
		Error(0x02,0x21,NULL,(void*)argv[i]); }

	if (!s) {
		for (i=0;i<NAME_MAX-4&&argv[1][i]!='.'&&argv[1][i]!='\0';i++);
		if (!(s=(char*)malloc(i+5))) Error(0x02,0x07,NULL,NULL);
		s[0]='\0';
		strncat(s,argv[1],i);
		strcat(s,".smc"); }
...
void Second_Pass (void) {
	...
	while ((r=Dll_Pull(Global_Link_R))) {
		if (!(d=Dll_Find(Global_Link,r))) Error(0x04,0x2A,NULL,(void*)r);
	...
	for (i=0;i<s;i++) fputc((char)j&0xFF,Out->file), j=j>>8; }
	return; }
...
void Do_Assembly (File_s *In) {
	...
	for(m=0;strcmp(ib,Set[m].mnemonic);)
		if(++m>=MNEMONIC_COUNT) Error(0x06,0x19,In,(void*)ib);

	// Identify Addressing Mode
	if (Advance_PC(1)) Error(0x06,0x28,In,NULL);
	if (Set[m].addr_m[0]!=-1) {
		// Implied
		In->data=0;
		In->size=0;
...
void Do_Compiler (File_s *In) {
	char *ib = In->line;
	if (ib[0]=='{') { Do_Link(In); return; }
	Error(0x07,0x1F,In,NULL); }
...
void Do_Link (File_s *In) {
	...
	if (!strcmp(ib,"+")||!strcmp(ib,"-")) {
		for( i=1,c=fgetc(f)
		   ; !feof(f)&&i<=NAME_MAX&&(Is_Namespace(c)||c=='+'||c=='-')
		   ; i++,c=fgetc(f)
		   )
			ib[i]=c;
		ib[i]='\0';
...
void Get_Token(File_s *In) {
 ...
 goto RETURN;
	...
	case '4': case '5': case '6': case '7': case '8': case '9':
		...
		if (c=='b'||c=='w'||c=='l') { ib[i++]=c; goto RETURN; }
		ungetc(c,f);
		break;
...
{	char digit[] = "0123456789ABCDEF";
 ...
 break;
	...
	case Token_number_t_:
		...
		switch(ib[i])
		{default:
			if(Is_Numberspace(ib[i])) { b=10; break; }
		...
		{	if(ib[i]==':') continue;
			...
			{	switch(b)
				{case 2: if(d>1) Error(0x1B,0x0C,In,(void*)ib); break;
				case 10: if(d>9) Error(0x1B,0x0E,In,(void*)ib); break;
				case 16: if(d>15) Error(0x1B,0x15,In,(void*)ib); break; }}
...
Dll_n* Dll_Find (Dll_n *l, Dll_n *t) {
 ...
 if (t) {
 // Initialize Find
 if(!l) Error(0x22,0x25,NULL,NULL); // Null list
 if(l->type!=Dll_head_t_) Error(0x22,0x08,NULL,NULL); // Headerless list
